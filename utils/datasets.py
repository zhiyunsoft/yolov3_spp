...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self,
        ...
        except Exception as e:  # 第一次运行，尚无shape文件，异常会跳转到这儿，进行shape文件的生成。
            # print("read {} failed [{}], rebuild {}.".format(sp, e, sp))
            # tqdm库会显示处理的进度
            # 读取每张图片的size信息
            if rank in [-1, 0]: # 单GPU为1，多GPU为0
        ...
        # 矩形方式训练（不是正方形）Rectangular Training https://github.com/ultralytics/yolov3/issues/232
        # 如果为true，训练网络时，会使用类似原图像比例的矩形(让最长边为img_size)，而不是img_size x img_size
        # 注意: 开启rect后，mosaic就默认关闭
        if self.rect:
    ...
    def __len__(self):  # 重写 Dataset父类的方法
